...
# full O2
    ...
    instructions_f=$suffix.$output_file.anlysis.instructions
    ...
    i=${#items[@]}
    for ((i=1; i<${#items[@]}; i+=1)); do
        # First half: elements from index 0 to i-1
        first_half=("${items[@]:0:i}")

        # Second half: elements from i to the end
        second_half=("${items[@]:i}")

        seq1=$(IFS=','; echo "${first_half[*]}")
        seq2=$(IFS=','; echo "${second_half[*]}")

        exe $OPT $build_dir/preopt.ll -o $build_dir/out.ll -S --enable-new-pm=1 --passes=$seq1
        exe $OPT --enable-new-pm=1 --passes="lowerswitch" $build_dir/out.ll -o $build_dir/out.ll
        ...
        ## Backend
        exe $OPT $build_dir/out.ll -o $build_dir/out.ll -S --enable-new-pm=1 --passes=$seq2
        ...
        # print the pass before which we are running ASPIS
        echo ${items[$i]} >> $passes_f
    done
    ...
    sed -E 's/.*\(([0-9]+)\).*/\1/' $percentages_f > tmpfile_percentages 
    sed -E 's/.*\/([0-9]+) .*/\1/' $percentages_f > tmpfile_instructions
    mv tmpfile_percentages $percentages_f
    mv tmpfile_instructions $instructions_f
