...
*/
Instruction* EDDI::cloneInstr(Instruction &I, std::map<Value *, Value *> &DuplicatedInstructionMap) {
...
*/
void EDDI::duplicateOperands (Instruction &I, std::map<Value *, Value *> &DuplicatedInstructionMap, BasicBlock &ErrBB) {
...
*/
void EDDI::addConsistencyChecks(Instruction &I, std::map<Value *, Value *> &DuplicatedInstructionMap, BasicBlock &ErrBB) {
      ...
      if (Duplicate != DuplicatedInstructionMap.end()) {
            ...
            for (int i=0; i<arraysize; i++) {
              ...
              if (OriginalElem->getType()->isPointerTy()) {
                ...
                if (OriginalElem->getType()->isFloatingPointTy()) {
                  CmpInstructions.push_back(B.CreateCmp(CmpInst::FCMP_UEQ, OriginalElem, CopyElem));
                }
                else {
        ...
        // else we just add a compare
          ...
          if (Original->getType()->isFloatingPointTy()) {
            CmpInstructions.push_back(B.CreateCmp(CmpInst::FCMP_UEQ, Original, Copy));
          }
          else {
...
void EDDI::fixFuncValsPassedByReference(Instruction &I, std::map<Value *, Value *> &DuplicatedInstructionMap, IRBuilder<> &B) {
...
void EDDI::duplicateGlobals (Module &Md, std::map<Value *, Value *> &DuplicatedInstructionMap) {
  Value *RuntimeSig;
  Value *RetSig;
...
*/
int EDDI::duplicateInstruction (Instruction &I, std::map<Value *, Value *> &DuplicatedInstructionMap, BasicBlock &ErrBB) {
...
*/
bool EDDI::isValueDuplicated(std::map<Value *, Value *> &DuplicatedInstructionMap, Instruction &V) {
...
Function *EDDI::duplicateFnArgs(Function &Fn, Module &Md, std::map<Value *, Value *> &DuplicatedInstructionMap) {
