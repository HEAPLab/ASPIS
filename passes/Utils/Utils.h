...
/**
 * TODO This function supports only one annotation for each function, multiple
 * annotations are discarded, perhaps I can fix this lol
 * @param Md The module where to look for the annotations
 * @param FuncAnnotations A map of Function and StringRef where to put the
 * annotations for each Function
 */
void getFuncAnnotations(Module &Md,
                        std::map<Value *, StringRef> &FuncAnnotations);

// Inserts the names of the compiled functions as a csv into the file passed as
// parameter
void persistCompiledFunctions(std::set<Function *> &CompiledFuncs,
                              const char *filename);

bool shouldCompile(
    Function &Fn, const std::map<Value *, StringRef> &FuncAnnotations,
...
StringRef getLinkageName(const LinkageMap &linkageMap,
